...
- CPU w STOP po Download → sprawdź Diagnostics buffer — prawdopodobnie błąd adresowania lub konfiguracji.
...
**Na rozmowie:** Jeśli pytają o „jak robisz diagnostykę maszyny” — wymień ProDiag obok Watch Table i Diagnostics Buffer. To pokaz, że znasz narzędzia nowszych wersji TIA Portal (V16+).
...
⚠️ Włączenie PUT/GET obniża poziom bezpieczeństwa — każde urządzenie znające IP PLC może czytać/pisać pamięć bez uwierzytelniania. **Nigdy nie włączaj** w systemach podłączonych do sieci korporacyjnej bez firewalla. Dla nowych integracji z IT preferuj OPC UA (TLS 1.2 + certyfikaty).

**Różnica od OPC UA:** S7/ISO on TCP = szybkie (<1ms), bez autentykacji. OPC UA = ~10ms, z szyfrowaniem i certyfikatami — wybór dla systemów IT/OT.
...
- **Global Library / Project Library z typami (PLCtype):** urządzenia (napędy, kurtyny, zawory) zaimplementowane jako wersjonowane typy biblioteczne — każda nowa instancja to przeciągnięcie z biblioteki, nie kopiowanie kodu. Zmiana w typie → aktualizacja wszystkich instancji jednym kliknięciem.
- **UDT jako interfejs danych:** zamiast pojedynczych tagów — struktury UDT dla każdego urządzenia (np. `typeValve`: cmd, status, alarm, mode). Dodanie nowego zaworu = dodanie zmiennej UDT, nie tworzenie 10 nowych tagów.
- **Tablice + pętle FOR w programie:** zamiast 20 identycznych sieci LAD — jeden FB parametryzowany indeksem, wywołany w pętli. Dodajesz urządzenie 21 przez zwiększenie tablicy i stałej `MAX_DEVICES`.
- **Symbolic addressing only (nie absolutne I0.0/Q0.0):** zmiana konfiguracji sprzętowej (nowa karta IO, zmiana slotu) nie łamie programu.
- **Hardware catalog z wolnymi slotami:** szafy ET200SP/MP zaprojektowane z rezerwą slotów i wolnymi adresami PROFINET — nowe moduły dodajesz bez przesypywania.
- **Modularny kod Safety:** każda strefa Safety jako osobny F-FB z własnym ACK i PASS_OUT — nowa strefa = nowa instancja FB, nie nowa logika.
- **Spójna konwencja nazw:** np. `++STATION_Drive01`, co pozwala na automatyczne generowanie dokumentacji i wyszukiwanie po wzorcu.
**Red flags braku skalowalności:** duplikacja kodu z ręczną edycją numerów (copy-paste FC z rename), absolutne adresy, brak bibliotek, każdy napęd w osobnym OB bez wspólnego FB wzorcowego.
